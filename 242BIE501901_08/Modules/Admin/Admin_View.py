...
class Add_products(tk.Frame):
    ...
    def validate_input(self): # Gọi hàm validate từ MatchBackend để kiểm tra các trường cơ bản
        data = AdminProcess_Main.validate_input(self)
        if not data:
            return False

        for i, price_entry in enumerate(self.price_entries): # Kiểm tra từng ô giá
            price = price_entry.get().strip()

            if not price:  #Nếu ô giá để trống, bỏ qua khu vực này
                continue

            if price.startswith("$"): # Kiểm tra định dạng giá
                price_value = price[1:]  # Bỏ ký tự $
                if not price_value.isdigit() or int(price_value) <= 0:
                    tk.messagebox.showerror("Error",
                                            f"Price for {self.TICKET_CATEGORIES[i]['area']} must be a positive integer (e.g., $123 or 123)!")
                    return False
            else:
                if not price.isdigit() or int(price) <= 0:  # Nếu không bắt đầu bằng $, kiểm tra toàn bộ chuỗi có phải là số nguyên dương không
                    tk.messagebox.showerror("Error",
                                            f"Price for {self.TICKET_CATEGORIES[i]['area']} must be a positive integer (e.g., $123 or 123)!")
                    return False
        return True
